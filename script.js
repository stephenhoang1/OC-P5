...
//FACE VALIDATION: RETURN A STATEMENT IF USER DOESN'T CHOOSE A FACE:
...
//QTY VALIDATION: RETURN A STATEMENT IF USER DOESN'T CHOOSE A QTY:
  var qty_is_clicked = false;
  //set the same variable to true if user clicks on a face.
...
$('.submit').on('click', function() {
  ...
  //if the user hasn't clicked on a FACE or QTY:
  //create a message telling them to do so, and stop program.
  if (icon_is_clicked == false) {
     $(".no_face_error").css("display", "block")
    return;
  } else if (qty_is_clicked == false) {
     $(".no_qty_error").css("display", "block")
    return;
  }

  //When user chooses a face upon receiving error message, remove error message.
  $(".no_face_error").css("display", "none");
   //When user chooses a qty upon receiving error message,, remove error message.
  $(".no_qty_error").css("display", "none");
